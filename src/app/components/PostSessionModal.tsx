...
export function PostSessionModal() {
    ...
    // Fetch session details to identify partner
    useEffect(() => {
        if (!roomid) return;
        fetchJsonWithAuth(`/api/match/${roomid}`)
            .then((data: any) => {
                if (data.success) {
                    // Logic based on identifying 'them' from the new match controller resolution
                    const partner = data.them;
                    setTargetId(partner.id);
                    setTargetName(partner.username || 'Anonymous User');
                }
            })
            .catch((err: any) => {
            ...
            <div className={`relative z-10 w-[800px] rounded-[3rem] overflow-hidden border shadow-2xl transition-all duration-300 transform
                ...
                <div className="px-16 py-14 text-center flex flex-col items-center">
                    ...
                    <div className="w-full max-w-md space-y-4">
                        {reportMode ? (
                            <div className={`p-8 rounded-[2rem] border relative overflow-hidden text-left mt-2 shadow-2xl transition-all duration-500 animate-in fade-in zoom-in-95
                                ...
                                {reportState === 'success' ? (
                                    <div className="flex flex-col items-center justify-center py-6 text-center">
                                        ...
                                        <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>You will no longer match with {targetName}.</p>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        <p className={`text-sm mb-5 leading-relaxed font-medium ${isDark ? 'text-red-300/80' : 'text-red-800'}`}>
                                            Reporting will block further matches with this user.
                                        </p>
                                        <textarea 
                                            rows={3}
                                            ...
                                            className={`w-full px-5 py-4 rounded-2xl text-sm outline-none border-2 transition-all resize-none mb-5
                                                ${isDark ? 'bg-[#0B1120] border-white/5 text-white' : 'bg-white border-red-200 text-slate-900'}`}
                                        />
                                        <div className="flex gap-4">
                                            <button 
                                                onClick={handleReportUser}
                                                disabled={reportState === 'submitting'}
                                                className="flex-[2] py-4 rounded-2xl font-black text-white bg-red-600 shadow-lg hover:bg-red-700 disabled:opacity-50">
                                                {reportState === 'submitting' ? 'Submitting...' : 'Block & Report'}
                                            </button>
                                            <button 
                                                onClick={() => setReportMode(false)}
                                                className={`flex-1 py-4 rounded-2xl font-bold border-2 ${isDark ? 'border-white/10 text-white' : 'border-slate-200 text-slate-500'}`}>
                                                Cancel
                                            </button>
                                        </div>
                                    </>
                            ...
                            <div className="space-y-4">
                                ...
                                {error && (
                                    <p className={`text-sm text-center font-semibold ${isDark ? 'text-red-400' : 'text-red-600'}`}>{error}</p>
                                )}
